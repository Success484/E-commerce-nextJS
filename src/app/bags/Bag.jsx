...
import { bag, coat, glass } from './page'
...
export default function Coat() {
  ...
  const handleIconClick = () => {
    ...
    const Bags = bag.map(bags => (
        <section key={bags.name} className={style.each}>

<div className='flex justify-between'>
        <button type='submit' onClick={handleIconClick}><IconColorChanger/></button>
        </div>
        <Link href='#'>
            <Image src={bags.image} height={300} width={300} alt={bags.name} className={style.size}/>
            <h2 className='text-center pb-[18px] hover:text-[#ff8f9c]'>{bags.name} <br />&#8358;{bags.price}</h2>
        </Link>
