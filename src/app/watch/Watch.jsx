...
export default function Watch() {
  ...
  const handleIconClick = () => {
    ...
    const watches = watch.map(watchs => (
        <section key={watchs.name} className={style.each}>

<div className='flex justify-between'>
        <button type='submit' onClick={handleIconClick}><IconColorChanger/></button>
        </div>
        <Link href='#'>
            <Image src={watchs.image} height={300} width={300} alt={watchs.name} className={style.size}/>
            <h2 className='text-center pb-[18px] hover:text-[#ff8f9c]'>{watchs.name} <br />&#8358;{watchs.price}</h2>
        </Link>
