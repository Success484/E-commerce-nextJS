...
export default function Coat() {
  ...
  const handleIconClick = () => {
    ...
    const Coats = coat.map(coat => (
        <section key={coat.name} className={style.each}>

<div className='flex justify-between'>
        <button type='submit' onClick={handleIconClick}><IconColorChanger/></button>
        </div>
        <Link href='#'>
            <Image src={coat.image} height={300} width={300} alt={coat.name} className={style.size}/>
            <h2 className='text-center pb-[18px] hover:text-[#ff8f9c]'>{coat.name} <br />&#8358;{coat.price}</h2>
        </Link>
