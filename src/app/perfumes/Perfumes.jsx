...
export default function Shoes() {
  ...
  const handleIconClick = () => {
    ...
    const Perfumes = perfume.map(perfumes => (
        <section key={perfumes.name} className={style.each}>

<div className='flex justify-between'>
        <button type='submit' onClick={handleIconClick}><IconColorChanger/></button>
        </div>
        <Link href='#'>
            <Image src={perfumes.image} height={300} width={300} alt={perfumes.name} className={style.size}/>
            <h2 className='text-center pb-[18px] hover:text-[#ff8f9c]'>{perfumes.name} <br />&#8358;{perfumes.price}</h2>
        </Link>
