...
export default function Cap() {
  ...
  const handleIconClick = () => {
    ...
    const faceCap = cap.map(caps => (
        <section key={caps.name} className={style.each}>

<div className='flex justify-between'>
        <button type='submit' onClick={handleIconClick}><IconColorChanger/></button>
        </div>
        <Link href='#'>
            <Image src={caps.image} height={300} width={300} alt={caps.name} className={style.size}/>
            <h2 className='text-center pb-[18px] hover:text-[#ff8f9c]'>{caps.name} <br />&#8358;{caps.price}</h2>
        </Link>
