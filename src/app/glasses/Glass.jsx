...
export default function Glass() {
  ...
  const handleIconClick = () => {
    ...
    const Glasses = glass.map(glasses => (
        <section key={glasses.name} className={style.each}>

<div className='flex justify-between'>
        <button type='submit' onClick={handleIconClick}><IconColorChanger/></button>
        </div>
        <Link href='#'>
            <Image src={glasses.image} height={300} width={300} alt={glasses.name} className={style.size}/>
            <h2 className='text-center pb-[18px] hover:text-[#ff8f9c]'>{glasses.name} <br />&#8358;{glasses.price}</h2>
        </Link>
